...
{
    template <>
    struct hash<z_engine::ZModel::Vertex>
    {
        std::size_t operator()(const z_engine::ZModel::Vertex& vertex) const
        {
            size_t seed = 0;
            z_engine::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}
...
namespace z_engine
{
    ...
    ZModel::ZModel(ZDevice& device, const Builder& builder) : device{device}
    {
