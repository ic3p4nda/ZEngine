...
{
    template<>
    struct hash<ZEngine::ZModel::Vertex>
    {
        std::size_t operator()(const ZEngine::ZModel::Vertex& vertex) const
        {
            size_t seed = 0;
            ZEngine::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}
